(function() {
    ...
    // =====================================================
    // 1. STOCKAGE INDEXEDDB + LOCALSTORAGE (FALLBACK)
    // =====================================================
    class PersistentStorage {
        ...
        init() {
            ...
            this.initPromise = new Promise((resolve, reject) => {
                ...
                request.onerror = (e) => {
                    console.warn('⚠️ IndexedDB indisponible, utilisation du fallback localStorage');
                    this.ready = false;
                    resolve(false);
                };
        ...
        _saveToLocalStorage(pages) {
            try {
                ...
                localStorage.setItem(this.fallbackKey, data);
            ...
            if (isReady) {
                try {
                    ...
                    if (pages && pages.length > 0) {
                        const hasImages = pages.some(p => p.imageData && p.imageData.startsWith('data:image'));
    ...
    function renderCurrentPage() {
        ...
        if (currentIndex === 0) {
            page.isCover = true;
            // Utiliser l'image personnalisée si elle existe, sinon la SVG par défaut
            const coverImage = page.imageData || COVER_DEFAULT;
            pageContent.innerHTML = `
        ...
        if (hasImage) {
            exerciseHtml = `
                    <img src="${page.imageData}" class="full-page-image" alt="Exercice" draggable="false" onclick="openLightbox('${page.imageData}')" />
                    <div class="click-hint">🔍 ${t('clickToZoom')}</div>
                `;
        } else {
            exerciseHtml = `
    ...
    // --- Télécharger la page en HTML A4 ---
    function downloadCurrentPage() {
        ...
        let bodyContent = '';
        if (currentIndex === 0) {
            const imgSrc = page.imageData || COVER_DEFAULT;
            bodyContent = `
    ...
    initBook();
    console.log('📖 Appixo Book · Consultation v2.1 · Image par défaut depuis images/cover-default.svg');
})();